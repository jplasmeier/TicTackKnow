...
public class Game extends JFrame
{
	...
	public int[][] wins = new int[28][4];
	
	public ArrayList<Integer> bad = new ArrayList<Integer>();
	...
	public Game()
	{
		...
		//Set layout
		content.setLayout(new GridLayout(6,5));

		//Create cells and handlers
		cells=new TTTButton[25];
		cellHandlers=new CellButtonHandler[25];
		for(int i=0; i<25; i++)
		{
			char ch=(char)('0'+i+1);
	...
	public void aiNext (int i) 
	{
		...
		System.out.println("Checking for bad moves");
		int badMove = log.checkForbiddenMoves(noughtsArray, crossesArray);
		System.out.println("Result of check: " + badMove);
		if(badMove != -1) {
			//remove bad move
			System.out.println("We know that " + badMove + " is a bad move.");
			bad.add(badMove);
		};
		noughts = !noughts;
	...
	private int MaxMove (int depth)
	{
		...
		if (moves.size() == 1)
		{
			...
			for (int k = 0; k < moves.size(); k++) 
			{
				...
				System.out.println();
				int value = analyze();
				if (bad.size() > 0) System.out.println("Bad move(s) detected! Move(s): " + bad.toString());
				System.out.println(minMove);
				if (value > bestMoveValue && !bad.contains(minMove))
				{
					bestMoveValue = value;
					bestMove = minMove;
				}
	...
	private int MinMove (int depth)
	{
		...
		if (moves.size() == 1)
		{
			...
			for (int k = 0; k < moves.size(); k++) 
			{
				...
				int value = analyze();
				if (bad.size() > 0) System.out.println("Bad move detected! Move: " + bad.toString());
				if (value < bestMoveValue && !bad.contains(maxMove))
				{
					bestMoveValue = value;
					bestMove = maxMove;
				}
	...
	private class CellButtonHandler implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			log.parseGameState(noughtsArray, crossesArray);
			...
			if(checkWinner(noughtsArray, crossesArray))
			{
				...
				if(noughts)
				{
					...
					log.writeGameResult(-1); //-1 means computer lost
					try {
						log.writeDataToLossLog();
				...
				else
				{
					...
					log.writeGameResult(1); //1 means computer won
				}
			}
			else if ((noughtsArray.size() + crossesArray.size()) == 25){
				gameOver = true;
				result.setText("Draw!");
				log.writeGameResult(0);
			}
			else
			{
				//Change player
